...
Usage:
    .pi/agent/sync-models.py
    .pi/agent/sync-models.py --include-all
    .pi/agent/sync-models.py --api-key KEY --ip 192.168.1.10 --port 8888
    .pi/agent/sync-models.py --config /path/to/models.json
...
the server's /v1/models response.
"""
...
def main() -> int:
    ...
    try:
        ...
        if len(providers) != 1:
            raise RuntimeError("expected exactly one provider in the config")
        provider_name, provider = next(iter(providers.items()))
        base_url = provider.get("baseUrl")
        api_key = provider.get("apiKey")
        if not base_url or not api_key:
            raise RuntimeError(f"provider '{provider_name}' needs baseUrl and apiKey")
        ...
        if args.ip or args.port:
            ...
            hostname = args.ip or parsed.hostname
            port = args.port or parsed.port
            if not hostname:
                raise RuntimeError("--ip is required when baseUrl has no hostname")
            netloc = hostname if port is None else f"{hostname}:{port}"
            base_url = urlunsplit((parsed.scheme or "http", netloc, parsed.path, "", ""))
